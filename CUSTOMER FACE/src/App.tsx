...
function BillingScreen({
  ...
  const [selectedPlan, setSelectedPlan] = useState<"monthly" | "biannual" | "annual">("biannual")
  const [selectedMethod, setSelectedMethod] = useState<"jazzcash" | "easypaisa" | "card" | "bank">("jazzcash")
  ...
  return (
    <div className="flex flex-col h-full screen-enter" style={{ background: "#F1F7F4" }}>
      {/* Header */}
      <header
        className="px-4 pt-10 pb-3 flex-shrink-0 flex items-center gap-3"
        style={{ background: "#F4FAF7", borderBottom: "1px solid #D5E2DD" }}
      >
        <button
          onClick={onBack}
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs truncate" style={{ color: "#52635F" }}>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        ...
        <div
          ...
          style={{
            background: "linear-gradient(135deg, #074D3E 0%, #087F63 100%)",
            color: "#fff",
            boxShadow: "0 4px 16px rgba(8,127,99,0.18)",
          }}
        >
          <div
            className="w-16 h-16 rounded-2xl bg-white/20 p-2 flex items-center justify-center flex-shrink-0"
            style={{ border: "1px solid rgba(255,255,255,0.3)" }}
          >
            <img src={iconSrc} alt={commodity} className="w-full h-full object-contain" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <span className="font-extrabold text-xl">{tc(commodity)}</span>
            ...
            <p className="text-xs text-emerald-100 mt-1">
              {lang === "ur"
                ? "تمام منڈیوں کے روزانہ ریٹس، تاریخی ڈیٹا اور الرٹس"
                : "Real-time rates, historic trends & alerts for 150+ mandis"}
            </p>
          </div>
        </div>
        ...
        {/* Plan Selector */}
        <div>
          <p className="font-bold text-sm text-[#183B34] mb-2.5">
            {lang === "ur" ? "پلان منتخب کریں" : "Select Subscription Plan"}
          </p>
          <div className="grid grid-cols-3 gap-2.5">
            {[
              { id: "monthly", title: lang === "ur" ? "۱ ماہ" : "1 Month", price: "Rs. 499", badge: "" },
              { id: "biannual", title: lang === "ur" ? "۶ ماہ" : "6 Months", price: "Rs. 2,499", badge: lang === "ur" ? "مقبول" : "Popular" },
              { id: "annual", title: lang === "ur" ? "۱ سال" : "1 Year", price: "Rs. 4,499", badge: lang === "ur" ? "بہترین بچت" : "Best Value" },
            ].map((p) => {
              ...
              return (
                <button
                  ...
                  className="tap-target relative p-3 rounded-2xl flex flex-col items-center justify-center text-center transition-all"
                  style={{
                    background: active ? "#E4F2EC" : "#FFFFFF",
                    border: active ? "2px solid #087F63" : "1.5px solid #D5E2DD",
                    boxShadow: active ? "0 4px 12px rgba(8,127,99,0.14)" : "none",
                  }}
                >
                  {p.badge && (
                    <span
                      className="absolute -top-2 px-1.5 py-0.2 rounded-full text-[9px] font-bold"
                      style={{ background: "#087F63", color: "#fff" }}
                    >
                      {p.badge}
                    </span>
                  )}
                  <span className="text-xs font-bold text-[#183B34]">{p.title}</span>
                  <span className="text-sm font-extrabold text-[#087F63] mt-1">{p.price}</span>
                </button>
              )
            })}
          </div>
        ...
        {/* Feature Highlights */}
        <div className="bg-white rounded-2xl p-4 border border-[#D5E2DD] space-y-2.5">
          <p className="font-bold text-xs text-[#52635F] uppercase tracking-wider">
            {lang === "ur" ? "شامل سہولیات" : "Included Features"}
          </p>
          {[
            lang === "ur" ? "تمام ۱۵۰+ غلہ منڈیوں کے براہِ راست ریٹس" : "Live prices across 150+ Punjab & Sindh mandis",
            lang === "ur" ? "روزانہ واٹس ایپ اور ایس ایم ایس الرٹس" : "Daily WhatsApp & SMS price change alerts",
            lang === "ur" ? "قیمت کے رجحانات اور اگلی پیشگوئیاں" : "Price trend analytics & 30-day forecast",
            lang === "ur" ? "منڈی آڑھتی اور بیوپاری رابطہ نمبرز" : "Direct mandi merchant directory & contacts",
          ].map((feat, idx) => (
            <div key={idx} className="flex items-center gap-2.5 text-xs text-[#183B34] font-medium">
              <span className="w-4 h-4 rounded-full bg-[#E4F2EC] text-[#087F63] flex items-center justify-center font-bold text-[10px]">
                ✓
              </span>
              <span>{feat}</span>
            </div>
        ...
        {/* Payment Methods */}
        <div>
          <p className="font-bold text-sm text-[#183B34] mb-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              { id: "card", name: lang === "ur" ? "بینک کارڈ" : "Debit/Credit Card" },
              { id: "bank", name: lang === "ur" ? "براہِ راست بینک" : "Direct Bank Transfer" },
            ].map((m) => {
              ...
              return (
                <button
                  ...
                  className="tap-target p-3 rounded-xl flex items-center gap-2 font-bold text-xs"
                  style={{
                    background: active ? "#E4F2EC" : "#FFFFFF",
                    border: active ? "1.5px solid #087F63" : "1.5px solid #D5E2DD",
                    color: active ? "#087F63" : "#52635F",
                  }}
                >
                  <span className={`w-3.5 h-3.5 rounded-full border flex items-center justify-center ${active ? "border-[#087F63] bg-[#087F63]" : "border-[#9AB0A6]"}`}>
        ...
        {/* Mobile Number for Account / Billing */}
        <div>
          <p className="font-bold text-xs text-[#52635F] mb-1">
            {lang === "ur" ? "موبائل نمبر (اکاؤنٹ اور الرٹس کے لیے)" : "Mobile Number (for alerts & billing)"}
          </p>
          <input
            type="tel"
            value={phone}
            onChange={(e) => setPhone(e.target.value)}
            className="w-full h-11 px-3.5 rounded-xl bg-white border border-[#D5E2DD] text-sm font-bold text-[#183B34] focus:outline-none focus:border-[#087F63]"
          />
        </div>
      </div>

      {/* Bottom Action */}
      <div className="p-4 bg-white border-t border-[#D5E2DD] flex-shrink-0 flex items-center justify-between gap-3">
        <div>
          <p className="text-[11px] text-[#52635F] font-semibold">{lang === "ur" ? "کل رقم:" : "Total Payable:"}</p>
          <p className="text-lg font-extrabold text-[#087F63]">
            {selectedPlan === "monthly" ? "Rs. 499" : selectedPlan === "biannual" ? "Rs. 2,499" : "Rs. 4,499"}
          </p>
        </div>
        <button
          onClick={handleSubscribe}
          className="tap-target flex-1 h-12 rounded-full bg-[#087F63] text-white font-extrabold text-sm flex items-center justify-center shadow-lg shadow-emerald-900/20"
        >
          {lang === "ur" ? "سبسکرائب اور فعال کریں" : "Subscribe & Activate"}
        </button>
      </div>
      ...
      {subscribedModal && (
        <div className="zm-sheet-overlay" style={{ zIndex: 300 }}>
          <div className="zm-sheet p-6 text-center space-y-4" style={{ background: "#FFFFFF", borderRadius: "24px 24px 0 0" }}>
            ...
            <div>
              <h2 className="text-xl font-extrabold text-[#183B34]">
                {lang === "ur" ? "سبسکرپشن کامیابی سے فعال ہو گئی!" : "Subscription Activated!"}
              </h2>
              <p className="text-xs text-[#52635F] mt-1.5">
                {lang === "ur"
                  ? `${tc(commodity)} کے تمام منڈی ریٹس اب آپ کے ہوم پیج پر دستیاب ہیں۔`
            ...
            <button
              ...
              className="tap-target w-full h-12 rounded-full bg-[#087F63] text-white font-bold text-sm"
            >
              {lang === "ur" ? "ہوم اسکرین پر جائیں" : "Go to Dashboard"}
            </button>
          </div>
        </div>
      )}
